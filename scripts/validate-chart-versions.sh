...
cd ${STABLE}
echo "📝 Checking for updated Chart versions since the last charts release $LAST_RELEASE_TAG"
for d in */; do
    ...
    echo "✅ Chart $d had no changes since the last charts release"
    continue
    fi
    ...
    echo "✅ Chart $d has a different version since the last charts release ($LAST_RELEASE_CHART_VERSION -> $LAST_COMMIT_CHART_VERSION)"
    fi
done
exit $EXIT_CODE
